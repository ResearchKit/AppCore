...
@class RKSTHealthCorrelationCollector;
 ...
 *
 */
@protocol RKSTStudyDelegate <NSObject>
 ...
 * @return Return NO if the collected objects could not be consumed (will stop further collection)
 */
- (BOOL)study:(RKSTStudy *)study healthCorrelationCollector:(RKSTHealthCorrelationCollector *)collector anchor:(NSNumber *)anchor didCollectObjects:(NSArray /* <HKSample> */ *)objects;
 ...
 */
@property (weak) id<RKSTStudyDelegate> delegate;

/**
 * @brief The identifier of the study.
 *
 * Initialized when the study is created with RKSTStudyManager.
 */
...
@interface RKSTStudy (RKSTCollector)

/**
 * @brief Array of any active RKSTCollector objects for this study.
 *
 ...
 */
- (RKSTHealthCollector *)addHealthCollectorWithSampleType:(HKSampleType *)sampleType unit:(HKUnit *)unit startDate:(NSDate *)startDate error:(NSError * __autoreleasing *)error;
 ...
 * @param startDate Samples should be collected starting at this date
 */
- (RKSTHealthCorrelationCollector *)addHealthCorrelationCollectorWithCorrelationType:(HKCorrelationType *)correlationType sampleTypes:(NSArray *)sampleTypes units:(NSArray *)units startDate:(NSDate *)startDate error:(NSError * __autoreleasing *)error;
 ...
 *
 */
- (RKSTMotionActivityCollector *)addMotionActivityCollectorWithStartDate:(NSDate *)startDate error:(NSError* __autoreleasing *)error;

/**
 * @brief Remove the specified collector
 */
- (BOOL)removeCollector:(RKSTCollector *)collector error:(NSError* __autoreleasing *)error;
 ...
 *
 * This method triggers running all the RKSTCollector collections associated with
