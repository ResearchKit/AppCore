...
/**
 * @brief An ORKDataLogger manages one "log" as a set of files in a directory.
 *
...
/**
 * @brief Convenience for a data logger with an ORKJSONLogFormatter.
 *
...
- (instancetype)initWithDirectory:(NSURL *)url logName:(NSString *)logName formatter:(ORKLogFormatter *)formatter delegate:(id<ORKDataLoggerDelegate>)delegate NS_DESIGNATED_INITIALIZER;

@property (weak) id<ORKDataLoggerDelegate> delegate;
...
/**
 * @brief Manager for a set of ORKDataLogger's
 *
 * @discussion This manager can be used to organize the ORKDataLogger logs in a directory,
...
- (instancetype)initWithDirectory:(NSURL *)directory delegate:(id<ORKDataLoggerManagerDelegate>)delegate NS_DESIGNATED_INITIALIZER;

@property (weak) id<ORKDataLoggerManagerDelegate> delegate;
 ...
 * @discussion Analogous to similar method on ORKDataLogger, but accepts an array of files
